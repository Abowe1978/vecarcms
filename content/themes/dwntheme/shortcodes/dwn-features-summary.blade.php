{{-- DWN Features Summary Section --}}
<section class="dwn-full-bleed dwn-features-summary py-10">
    <div class="container text-center">
        <h4 class="fs-1 fw-bold mb-3">{{ $title ?? 'Explore our features' }}</h4>
        <p class="text-muted mb-5">{{ $subtitle ?? 'Build landing pages faster with powerful tools and integrations.' }}</p>
        @php
            $items = $items ?? [
            ...
            @foreach($items as $item)
                <div class="col-12 col-md-6 col-lg-4 d-flex flex-column justify-content-start" data-aos="fade-in">
                    <span class="f-w-8 d-block text-primary">
                        <img src="{{ theme_asset('assets/images/' . $item['icon']) }}" alt="{{ $item['title'] }}" class="img-fluid" style="max-width: 48px;">
                    </span>
                    <p class="fw-medium mb-1 mt-3 fs-5">{{ $item['title'] }}</p>
                    <span class="text-muted fs-7">{{ $item['description'] }}</span>
                </div>
            @endforeach
        </div>
        <a href="{{ $button_url ?? '#' }}" class="btn btn-primary d-table mx-auto mt-5 w-100 w-md-auto mt-lg-8 mb-3" role="button">
            {{ $button_text ?? 'Start your trial' }}
        </a>
