...
<div class="max-w-7xl mx-auto py-6">
    <div class="mb-6 flex items-center space-x-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.admins.edit') }}: {{ $admin->name }}</h1>
        <div>
            @if($admin->hasRole('developer'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                    <i class="fas fa-code mr-1.5"></i> Developer
                </span>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <form action="{{ route('admin.admins.update', $admin) }}" method="POST" enctype="multipart/form-data" class="divide-y divide-gray-200">
            @csrf
            @method('PUT')
                ...
                <div class="flex flex-col md:flex-row items-start md:items-center">
                        ...
                        <div class="relative group">
                            <div class="w-32 h-32 rounded-full border-4 border-white shadow-md overflow-hidden bg-gray-100">
                                @if($admin->profile_image)
                                    <img src="{{ asset('storage/' . $admin->profile_image) }}" alt="{{ $admin->name }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <label for="profile_image" class="absolute inset-0 flex items-center justify-center w-full h-full bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 rounded-full transition-opacity duration-200 cursor-pointer">
                                <span class="text-white text-sm font-medium"><i class="fas fa-camera mr-1"></i> {{ __('Change') }}</span>
                            </label>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            {{ __("Update the administrator's profile information and role.") }}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="text" 
                                   ...
                                   required
                                   {{ $admin->hasRole('developer') && !Gate::allows('manage_roles') ? 'readonly' : '' }}>
                        </div>
                        @error('name')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label for="password" class="block text-sm font-medium text-gray-700">
                            {{ __('admin.admins.password') }} <span class="text-sm font-normal text-gray-500">({{ __('optional') }})</span>
                        </label>
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            ...
                            <input type="password" 
                                   ...
                                   {{ $admin->hasRole('developer') && !Gate::allows('manage_roles') ? 'disabled' : '' }}
                ...
                <div class="mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @php
                            ...
                            $currentRole = $admin->roles->first() ? $admin->roles->first()->name : '';
                        @endphp
                        ...
                        @foreach($roles as $role)
                            ...
                            <div class="relative flex group">
                                <input type="radio" 
                                       ...
                                       {{ $admin->hasRole('developer') ? 'disabled' : '' }}
                                       required>
                        ...
                        <div class="relative">
                            ...
                            <label class="flex p-4 border border-purple-500 rounded-lg bg-purple-50 ring-2 ring-purple-500">
                                <div class="flex items-center">
                                    <div class="w-8 h-8 mr-4 flex items-center justify-center rounded-full bg-purple-100 text-purple-800">
                                        <i class="fas fa-code"></i>
                                    </div>
                                    <div>
                                        <p class="font-medium">Developer</p>
                                        <p class="text-sm text-gray-500">{{ __('admin.roles.developer_description') }}</p>
                                    </div>
                                </div>
                            </label>
                        </div>
            ...
            <div class="px-4 py-4 sm:px-6 flex justify-end space-x-3 bg-gray-50">
                ...
                <button type="submit" 
                        ...
                        {{ $admin->hasRole('developer') && !Gate::allows('manage_roles') ? 'disabled' : '' }}>
                    <i class="fas fa-save mr-2"></i> {{ __('admin.admins.update') }}
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const previewImage = imageInput.closest('.relative').querySelector('img') || null;
        const placeholderIcon = imageInput.closest('.relative').querySelector('.fas.fa-user') || null;
